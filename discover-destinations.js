...
const DESTS=[
 {id:'las-vegas',name:'Las Vegas',region:'Nevada, USA',wiki:'Las Vegas',tag:'ICONIC CITY',theme:'city'},
 {id:'rome',name:'Rome',region:'Italy',wiki:'Rome',tag:'HISTORY & CULTURE',theme:'heritage'},
 {id:'bahamas',name:'Bahamas',region:'Caribbean',wiki:'The Bahamas',tag:'ISLAND ESCAPE',theme:'sea'},
 {id:'miami',name:'Miami',region:'Florida, USA',wiki:'Miami',tag:'SUN & ART DECO',theme:'sea'},
 {id:'london',name:'London',region:'England',wiki:'London',tag:'CITY STORIES',theme:'city'},
 {id:'paris',name:'Paris',region:'France',wiki:'Paris',tag:'ROMANCE & CULTURE',theme:'heritage'},
 {id:'new-york',name:'New York',region:'USA',wiki:'New York City',tag:'BIG CITY ENERGY',theme:'city'},
 {id:'venice',name:'Venice',region:'Italy',wiki:'Venice',tag:'TIMELESS BEAUTY',theme:'heritage'}
];
const icon={
...
function badge(d,size='card'){return `<span class="mu-dest-badge mu-dest-badge-${size} mu-dest-badge-${d.id}" aria-hidden="true">${icon[d.id]||''}<b>${safe(d.name)}</b></span>`;}
...
function card(d,i){const isSaved=saved().has(d.id);return `<article class="mu-dest-card mu-dest-${d.theme}" data-dest-card="${d.id}">
  <button class="mu-dest-photo" type="button" data-dest-open="${d.id}" data-dest-wiki="${safe(d.wiki)}" aria-label="Discover ${safe(d.name)}">
...
function render(){
 ...
 host.innerHTML=`<section class="mu-discover-hub">
  <header class="mu-discover-hero"><span class="mu-discover-orbit">⌕</span><div><small>DISCOVER</small><h2>Where will your story go next?</h2><p>Iconic places, dream destinations and bucket-list ideas — ready to become your next journey.</p></div></header>
