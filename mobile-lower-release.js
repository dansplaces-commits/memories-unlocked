/* Memories Unlocked — release lower-card renderer. Embedded artwork, no external asset dependency. */
(function(){
...
function apply(){
 ...
 jcards.slice(0,2).forEach((card,i)=>{const cover=card.querySelector('.mu-mm-cover');const hasReal=cover?.dataset.mmJcover&&cover.classList.contains('has-photo')&&!cover.classList.contains('mu-release-art')&&(cover.style.backgroundImage||'').includes('supabase');if(!hasReal)paint(cover,J,'200% 100%',i===0?'0% 50%':'100% 50%');});
 let prompts=[...memories.querySelectorAll('.mu-mm-memory-prompt')];const real=[...memories.querySelectorAll('.mu-mm-memory-card')];
 while(real.length+prompts.length<3){const i=Math.min(real.length+prompts.length,2);const p=memoryPrompt(i);memories.appendChild(p);prompts.push(p);}
 ...
 const ready=covers.length===2&&photos.length===3&&covers.every(el=>(el.style.backgroundImage||'').length>20)&&photos.every(el=>(el.style.backgroundImage||'').length>20);
...
function start(){apply();const root=document.querySelector('.mu-mobile-master-home')||document.getElementById('home');if(root)new MutationObserver(m=>{if(m.some(x=>x.addedNodes?.length||x.removedNodes?.length))schedule();}).observe(root,{childList:true,subtree:true});let n=0;const t=setInterval(()=>{if(apply()||++n>40)clearInterval(t);},100);window.addEventListener('pageshow',schedule,{passive:true});}
